...
export default new class Drive{
    ...
    async sendRequest(messageText){
        ...
        cipher.start({iv});
        cipher.update(forge.util.createBuffer(messageText));
        cipher.finish();
        const encryptedMessage = cipher.output.getBytes();
        
        const hmac = forge.hmac.create();
        hmac.start('sha256', this.hmac);
        hmac.update(iv + encryptedMessage);
        const hmacResult = hmac.digest().getBytes();

        let response;
        try{ response = await fetch('/', {method:'POST', body:byteStringToUint8Array(encryptedMessage), headers:{'x-iv':window.btoa(iv), 'x-hmac':window.btoa(hmacResult)}}); }
